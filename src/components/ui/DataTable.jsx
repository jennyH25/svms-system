...
export const TableCellText = ({ primary, secondary }) => (
  <div>
    ...
    {secondary && (
      <p className="text-gray-500 text-[13px] font-medium">{secondary}</p>
    )}
  </div>
);
...
export const TableCellDateTime = ({ date, time }) => (
  <div>
    ...
    {time && (
      <span className="text-cyan-600 text-[13px] font-medium ml-2">{time}</span>
    )}
  </div>
);
...
export const TableCellBadge = ({ label, variant = "default" }) => {
  ...
  return (
    <span
      className={`inline-flex px-3 py-1 rounded-full text-[12px] font-medium ${variants[variant]}`}
    >
      {label}
    </span>
  );
