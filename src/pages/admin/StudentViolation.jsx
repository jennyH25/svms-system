...
import { Plus, TrendingUp, TrendingDown, Search, ChevronDown, Download, Paperclip, Check } from 'lucide-react'
import {
  DropdownMenu,
  DropdownMenuTrigger,
  DropdownMenuContent,
...
const StudentViolation = () => {
  ...
  const actions = [
    {
      label: 'Edit',
      icon: <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.232 5.232l3.536 3.536M9 13h3l8-8a2.828 2.828 0 10-4-4l-8 8v3h3z" /></svg>,
      onClick: (row) => console.log('Edit', row),
    },
    {
      label: 'Delete',
      icon: <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3m5 0H6" /></svg>,
  ...
  const handleClearStatus = (rowId) => {
    if (window.confirm('Are you sure you want to mark this violation as cleared?')) {
      const now = new Date();
  ...
  const sampleUsers = [
    {
      ...
      remarks: 'Lorem Ipsum',
      signature: (
        <Button size="sm" variant="secondary" className="px-3 py-1 gap-2" onClick={() => handleAttachSignature(1)}>
    ...
    },
    {
      ...
      signature: (
        <Button size="sm" variant="secondary" className="px-3 py-1 gap-2" onClick={() => handleAttachSignature(2)}>
          <Paperclip className="w-4 h-4" /> Attach
        </Button>
      ),
    ...
    },
    {
      ...
      remarks: 'Cheating',
      signature: (
        <Button size="sm" variant="secondary" className="px-3 py-1 gap-2" onClick={() => handleAttachSignature(3)}>
          <Paperclip className="w-4 h-4" /> Attach
        </Button>
      ),
    ...
    },
    {
      ...
      remarks: 'Absent',
      signature: (
        <Button size="sm" variant="secondary" className="px-3 py-1 gap-2" onClick={() => handleAttachSignature(4)}>
          <Paperclip className="w-4 h-4" /> Attach
        </Button>
      ),
    ...
    },
    {
      ...
      remarks: 'Plagiarism',
      signature: (
        <Button size="sm" variant="secondary" className="px-3 py-1 gap-2" onClick={() => handleAttachSignature(5)}>
          <Paperclip className="w-4 h-4" /> Attach
        </Button>
      ),
    ...
    <div className="text-white">
      ...
      <div className="grid grid-cols-2 gap-4 mt-6 mb-6 w-full h-full">
        ...
        <AnimatedContent delay={0.1}>
          <StatCard title="At-Risk Students" value={0} percentage={0} icon={<TrendingUp />} className="col-span-1 min-w-[220px] h-full w-full" />
        </AnimatedContent>
        <AnimatedContent delay={0.2}>
          <StatCard title="Violations" value={0} percentage={11.01} icon={<TrendingDown />} className="col-span-1 min-w-[220px] h-full w-full" />
        </AnimatedContent>
        </div>
      </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent>
                ...
                <DropdownMenuItem onClick={() => handleYearChange("4th Year")}>4th Year</DropdownMenuItem>
              </DropdownMenuContent>
            </DropdownMenu>
            {/* Status Dropdown */}
            <DropdownMenu>
