...
export async function syncAuthDatabase(options = {}) {
  ...
  await dbPool.query(`
    CREATE TABLE IF NOT EXISTS users (
      ...
      role VARCHAR(20) NOT NULL CHECK (role IN ('admin', 'super_admin', 'both', 'student')),
  ...
  await dbPool.query(`
    ...
    CHECK (role IN ('admin', 'super_admin', 'both', 'student'))
  `);
...
export async function syncStudentsDatabase() {
  ...
  await dbPool.query(`
    CREATE TABLE IF NOT EXISTS "Students" (
      ...
      archive_warning_week_sent_at TIMESTAMPTZ,
      archive_warning_day_sent_at TIMESTAMPTZ,
      created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
      updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
    )
  `);
  ...
  await dbPool.query(`
    ALTER TABLE "Students"
    ADD COLUMN IF NOT EXISTS archived_school_year VARCHAR(20)
  `);

  await dbPool.query(`
    ALTER TABLE "Students"
    ADD COLUMN IF NOT EXISTS archive_warning_week_sent_at TIMESTAMPTZ
  `);

  await dbPool.query(`
    ALTER TABLE "Students"
    ADD COLUMN IF NOT EXISTS archive_warning_day_sent_at TIMESTAMPTZ
  `);
  ...
  await dbPool.query(`
    CREATE INDEX IF NOT EXISTS students_archive_retention_idx
    ON "Students" (is_archived, archived_at, archived_school_year)
  `);
...
export async function syncArchiveRetentionDatabase() {
  if (!hasDbConfig()) {
    throw new Error(
      `Missing required environment variables: ${getMissingDbVars().join(", ")}`,
    );
  ...
  await dbPool.query(`
    CREATE TABLE IF NOT EXISTS archive_school_year_retention_notices (
      school_year VARCHAR(20) PRIMARY KEY,
      scheduled_deletion_at TIMESTAMPTZ NOT NULL,
      week_notice_sent_at TIMESTAMPTZ,
      day_notice_sent_at TIMESTAMPTZ,
  ...
  await dbPool.query(`
    CREATE TABLE IF NOT EXISTS admin_archive_notice_dismissals (
      id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
      ...
      created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
      updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
      UNIQUE (admin_user_id, school_year)
    )
  `);

  await dbPool.query(`
    CREATE INDEX IF NOT EXISTS archive_school_year_retention_deleted_idx
    ON archive_school_year_retention_notices (deleted_at, scheduled_deletion_at)
  `);

  await dbPool.query(`
    CREATE INDEX IF NOT EXISTS admin_archive_notice_dismissals_user_idx
    ON admin_archive_notice_dismissals (admin_user_id, school_year)
  `);
  ...
  await dbPool.query(`
    DROP TRIGGER IF EXISTS trg_archive_school_year_retention_updated_at
    ON archive_school_year_retention_notices
  `);

  await dbPool.query(`
    CREATE TRIGGER trg_archive_school_year_retention_updated_at
    BEFORE UPDATE ON archive_school_year_retention_notices
    FOR EACH ROW
    EXECUTE FUNCTION set_archive_retention_updated_at()
  `);

  await dbPool.query(`
    DROP TRIGGER IF EXISTS trg_admin_archive_notice_dismissals_updated_at
    ON admin_archive_notice_dismissals
  `);

  await dbPool.query(`
    CREATE TRIGGER trg_admin_archive_notice_dismissals_updated_at
    BEFORE UPDATE ON admin_archive_notice_dismissals
    FOR EACH ROW
    EXECUTE FUNCTION set_archive_retention_updated_at()
  `);
...
export async function syncEmailUsageDatabase() {
  if (!hasDbConfig()) {
    throw new Error(
      `Missing required environment variables: ${getMissingDbVars().join(", ")}`,
    );
  ...
  await dbPool.query(`
    ALTER TABLE email_send_logs
    ADD COLUMN IF NOT EXISTS context_label VARCHAR(100)
  `);
  ...
  await dbPool.query(`
    CREATE OR REPLACE FUNCTION normalize_email_send_log_recipients()
    RETURNS TRIGGER AS $$
    BEGIN
      IF NEW.recipients IS NOT NULL THEN
        NEW.recipients = jsonb_strip_nulls(NEW.recipients);
        IF NEW.recipients = '[]'::jsonb THEN
          NEW.recipients = NULL;
        END IF;
      END IF;
      RETURN NEW;
  ...
  await dbPool.query(`
    DROP TRIGGER IF EXISTS trg_email_send_logs_normalize_recipients ON email_send_logs
  `);

  await dbPool.query(`
    CREATE TRIGGER trg_email_send_logs_normalize_recipients
    BEFORE INSERT OR UPDATE ON email_send_logs
    FOR EACH ROW
    EXECUTE FUNCTION normalize_email_send_log_recipients()
  `);
